...
import sys
import json
import time  # [MỚI] Thêm thư viện time để hãm tốc độ
...
def run_rag_pipeline(inputs: dict) -> dict:
    time.sleep(2.5)  # [MỚI] Nghỉ 2.5s trước khi chạy RAG để né Rate Limit
...
def _call_judge(prompt: str, schema_class, retries=3) -> dict:
    for attempt in range(retries):
        try:
            time.sleep(2.5)  # [MỚI] Nghỉ 2.5s trước khi Giám khảo chấm điểm
            ...
            time.sleep(5)  # Nếu dính lỗi 429, nghỉ 5s rồi thử lại
    
    # Nếu thử 3 lần vẫn thất bại, trả về điểm 0 an toàn
    print("❌ Bỏ qua test case này do quá tải API.")
    return {"context_recall": 0, "context_precision": 0.0, "strict_faithfulness": 0, "answer_relevance": 0.0, "reasoning": "Rate Limit Exceeded"}

def evaluate_retrieval(run, example) -> list:
    # [MỚI] Check an toàn tránh lỗi KeyError: 'context'
    ...
    prompt = RETRIEVAL_EVAL_PROMPT.format(question=question, ground_truth=ground_truth, context=context)
...
def evaluate_generation(run, example) -> list:
    ...
    prompt = GENERATION_EVAL_PROMPT.format(question=question, context=context, generated_answer=generated_answer)
...
# =====================
# 5. KÍCH HOẠT EXPERIMENT
# =====================
if __name__ == "__main__":
    DATASET_NAME = "TechCorp_IT_Onboarding_GT"
    
    print(f"\n🚀 BẮT ĐẦU CHẠY EXPERIMENT TRÊN DATASET: {DATASET_NAME} (Chế độ chậm & an toàn)")
    print("-" * 50)
    ...
    experiment_results = evaluate(
        ...
        max_concurrency=1, # [MỚI] Ép chạy từng câu một, cấm chạy song song
    )
